...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // active btcking difficulty ajustment
    if(IsBCKINGForkEnabled(params, pindexLast->nHeight+1)) {
        return GetNextBCKINGWorkRequired(pindexLast, pblock, params);
    }

    return UintToArith256(params.powLimitBeforeFork).GetCompact();
}
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits, const Consensus::Params& params)
{
    ...
    // Check range
    if (fNegative || bnTarget == 0 || fOverflow || bnTarget > UintToArith256(params.powLimitBeforeFork))
        return false;
                                  ...
                                  const Consensus::Params &params)
{
    ...
    const CBlockIndex *pindexFirst_t = pCurIndex->GetAncestor(params.BCKINGHeight - 146);
    const CBlockIndex *pindexLast_t =  pCurIndex->GetAncestor(params.BCKINGHeight-2);
...
unsigned int GetNextBCKINGWorkRequired(const CBlockIndex *pindexPrev,
    ...
    if (IsBCKINGForkEnabled(params, nCurHeight) && nCurHeight - 149 < params.BCKINGHeight) {
        return UintToArith256(params.powLimit).GetCompact();
    } else {
...
arith_uint256 ComputeTarget(const CBlockIndex *pindexFirst,
    ...
    if (nActualTimespan > 288 * params.nPowTargetSpacing) {
        ...
        if (Curheight > params.BCKINGHeight + 500) { //
...
bool IsBCKINGForkEnabled(const Consensus::Params& params, const int height) {
    return height >= params.BCKINGHeight;
}
